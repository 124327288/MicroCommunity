package com.java110.api.listener.order;
 ...
 * 查询小区侦听类
 */
@Java110Listener("listOrdersListener")
public class ListOrdersListener extends AbstractServiceApiListener {
    ...
    protected void doSoService(ServiceDataFlowEvent event, DataFlowContext context, JSONObject reqJson) {
        ...
        OrderDto orderDto = BeanConvertUtil.covertBean(reqJson, OrderDto.class);

        int count = orderInnerServiceSMOImpl.queryOrdersCount(orderDto);

        List<ApiOrgDataVo> orgs = null;

        if (count > 0) {
            orgs = BeanConvertUtil.covertBeanList(orderInnerServiceSMOImpl.queryOrders(orderDto), ApiOrgDataVo.class);
        } else {
            orgs = new ArrayList<>();
        }
